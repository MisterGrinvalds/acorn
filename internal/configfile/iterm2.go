...
// ITermWriter implements the Writer interface for iTerm2 Dynamic Profiles.
// It generates JSON profile files from declarative YAML configuration.
type ITermWriter struct{}

func init() {
	Register(&ITermWriter{})
}

// Format returns the format identifier.
func (w *ITermWriter) Format() string {
	return "iterm2"
}

// Write generates iTerm2 dynamic profile JSON from values.
func (w *ITermWriter) Write(values map[string]interface{}) ([]byte, error) {
	profile := make(map[string]interface{})

	// Process each section
	if p, ok := values["profile"].(map[string]interface{}); ok {
		w.processProfile(profile, p)
	}
	if f, ok := values["font"].(map[string]interface{}); ok {
		w.processFont(profile, f)
	}
	if t, ok := values["terminal"].(map[string]interface{}); ok {
		w.processTerminal(profile, t)
	}
	if c, ok := values["cursor"].(map[string]interface{}); ok {
		w.processCursor(profile, c)
	}
	if i, ok := values["input"].(map[string]interface{}); ok {
		w.processInput(profile, i)
	}
	if m, ok := values["mouse"].(map[string]interface{}); ok {
		w.processMouse(profile, m)
	}
	if b, ok := values["behavior"].(map[string]interface{}); ok {
		w.processBehavior(profile, b)
	}
	if c, ok := values["colors"].(map[string]interface{}); ok {
		w.processColors(profile, c)
	}
	if km, ok := values["keyboard_maps"].([]interface{}); ok {
		w.processKeyboardMaps(profile, km)
	}
	if tags, ok := values["tags"].([]interface{}); ok {
...
func (w *ITermWriter) processProfile(profile, p map[string]interface{}) {
...
func (w *ITermWriter) processFont(profile, f map[string]interface{}) {
	family := "Menlo"
	size := 14
...
func (w *ITermWriter) processTerminal(profile, t map[string]interface{}) {
...
func (w *ITermWriter) processCursor(profile, c map[string]interface{}) {
...
func (w *ITermWriter) cursorTypeToInt(ct string) int {
...
func (w *ITermWriter) processInput(profile, i map[string]interface{}) {
...
func (w *ITermWriter) optionKeyToInt(mode string) int {
...
func (w *ITermWriter) processMouse(profile, m map[string]interface{}) {
...
func (w *ITermWriter) processBehavior(profile, b map[string]interface{}) {
...
func (w *ITermWriter) workingDirToString(mode string) string {
	switch strings.ToLower(mode) {
	case "home":
		return "Home"
	case "custom":
...
func (w *ITermWriter) processColors(profile, c map[string]interface{}) {
	// Check for scheme reference first
	if scheme, ok := c["scheme"].(string); ok {
		if cs, exists := ITerm2ColorSchemes[scheme]; exists {
	...
	if ansi, ok := c["ansi"].([]interface{}); ok && len(ansi) == 16 {
...
func (w *ITermWriter) applyColorScheme(profile map[string]interface{}, cs ITerm2ColorScheme) {
...
func (w *ITermWriter) hexToITermColor(hex string) map[string]interface{} {
...
func (w *ITermWriter) processKeyboardMaps(profile map[string]interface{}, kms []interface{}) {
	keymap := make(map[string]interface{})

	for _, km := range kms {
		if m, ok := km.(map[string]interface{}); ok {
...
func (w *ITermWriter) processTags(profile map[string]interface{}, tags []interface{}) {
