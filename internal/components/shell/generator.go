...
func (g *Generator) generatePaths(b *strings.Builder, paths []config.PathEntry) {
	for _, p := range paths {
...
func (g *Generator) generateWrappers(b *strings.Builder, wrappers []config.Wrapper) {
...
func (g *Generator) GenerateComponent(cfg *config.BaseConfig) *Component {
	return &Component{
		Name:        cfg.Name,
...
func (g *Generator) generateFunctionsString(cfg *config.BaseConfig) string {
	var b strings.Builder
...
func (g *Generator) GenerateConfigFiles(cfg *config.BaseConfig) ([]*configfile.GeneratedFile, error) {
	if len(cfg.Files) == 0 {
		return nil, nil
	}

	// Filter files by platform
	var filteredFiles []config.FileConfig
